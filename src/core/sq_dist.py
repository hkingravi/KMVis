...
def kernel(data1, data2, k_type):
    ...
    if (k_name == "polynomial" or k_name == "gaussian" or
        ...
        k_mat = kernel_base(data1, data2, k_type, k_use_speed)
    elif k_name == "locally_periodic":
        k_name1 = "gaussian"
        k_name2 = "periodic"
        k_params1 = array([k_params[0]])
        k_params2 = k_params
        k_type_gauss = KernelType(k_name1,k_params1, k_use_speed)
        k_type_periodic = KernelType(k_name2,k_params2, k_use_speed)
        k_mat = multiply(kernel_base(data1, data2, k_type_periodic, k_use_speed),kernel_base(data1, data2, k_type_gauss, k_use_speed))
    else:
